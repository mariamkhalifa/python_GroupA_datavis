...
categories = [] # strip out the frst row of text
installs = [] # push the insatlls adta here
ratings = [] # psh the ratings data here
...
with open("data/googleplaystore.csv") as csvfile:
    ...
    for row in reader:
        if line_count is 0: # strip teh headers out
            print('pushing text row to categories array')
            categories.append(row)
            line_count += 1
        else:
            # collect teh ratings info
            ratingsData = row[2]
